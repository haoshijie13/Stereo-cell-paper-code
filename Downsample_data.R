...
haoshijie@cngb-xcompute-0-9 14:02:19 /hwfssz1/ST_SUPERCELLS/P22Z10200N0661/haoshijie/FIGURE2/02.saturation
$ cat extract.R
library(data.table)
mask=fread("B03404C4.cell_mask.csv")
readcount=fread("B03404C4_raw_barcode_gene_exp.txt")
mergecount=merge(readcount,mask,by=c("x","y"))
mergecount=mergecount[,c("x","y","geneIndex","MIDIndex","readCount","cell")]
...
i=28000/x
res=parApply(cl,X=as.matrix(mergecount),FUN=function(xx){sum(sample(0:1,size=xx[5],replace=T,prob=c(1-i,i)))},MARGIN=1)
mergecount$downsample_reads=res
mergecount=mergecount[mergecount$downsample_reads!=0,]

fwrite(mergecount,file="downsample.B03404C4_raw_barcode_gene_exp.txt",row.names=T)
